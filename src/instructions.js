...
const instructionPageIdByGroup = {
  ...
  [GROUP_TYPE.AI_REVEAL_ONE]: [...commonPages.pageId, 8, 9, 10, 11].sort(
    (a, b) => a - b
  ),
...
function getVideoDescription(pageId) {
  if (pageId === 3) {
    return `<p>You can drag or swap objects, or click RESET to return to the original state.</p>`;
  }
  if (pageId === 6) {
    return `<p>If your first submission is not fully correct, 
  ...
  if (pageId === 8) {
    if (isAILimitedGroup()) {
      return `<p><strong>An AI agent is available to assist you during Phase 2 only.</strong></p>
        <p>The AI agent is available on demand, <strong>limited to one use per problem.</strong>
...
function loadInstructionsHTML(html) {
  ...
  instructionPages.forEach((pageId) => {
    ...
    const pageEl = createInstructionPage({
      ...
      type: pageType,
      src,
      description: getVideoDescription(pageId),
    });
